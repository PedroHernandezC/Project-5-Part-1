...
int main() {
    ...
    */
    ...
    const auto csvPath2 = csvDir + "/" + profileName + ".csv";
    std::ofstream csv_out(csvPath2);
    if (!csv_out.is_open()) {
        std::cerr << "failed to open " << csvPath2 << " for writing.\n";
    } else {
        ...
        for (const auto& run : runResults) {
            ...
            csv_out << run.to_csv_row() << std::endl;
        }
    }
    ...
    const auto profileName2 = std::string("batch_then_drain");
    const auto traceDir2 = std::string("../../traces") + "/" + profileName2;

    std::vector<std::string> traceFiles2;
    find_trace_files_or_die(traceDir2, profileName2, traceFiles2);

    if (traceFiles2.size() == 0) {
        std::cerr << "No trace files found.\n";
        exit(1);
    }

    std::vector<RunResult> runResults2;
    for (auto traceFile2: traceFiles2) {
        const auto pos = traceFile2.find_last_of("/\\");
        auto traceFileBaseName2 = (pos == std::string::npos) ? traceFile2 : traceFile2.substr(pos + 1);

        std::vector<Operation> operations2;
        RunMetaData run_meta_data2;
        load_trace_strict_header(traceFile2, run_meta_data2, operations2);

        if (run_meta_data2.N < 1 << 16) {
            RunResult oneRunResult_i0(run_meta_data2);
            QuadraticOracle oracle(compare_pair);
            oneRunResult_i0.impl = std::string("quadratic_oracle");
            oneRunResult_i0.trace_path = traceFileBaseName2;
            run_trace_ops(oracle, oneRunResult_i0, operations2);
            runResults2.emplace_back(oneRunResult_i0);
        }

        RunResult oneRunResult_i1(run_meta_data2);
        BinaryHeapInVector binaryHeap(compare_pair);
        oneRunResult_i1.impl = std::string("binary_heap");
        oneRunResult_i1.trace_path = traceFileBaseName2;
        run_trace_ops(binaryHeap, oneRunResult_i1, operations2);
        runResults2.emplace_back(oneRunResult_i1);

        RunResult oneRunResult_i2(run_meta_data2);
        BinomialQueue binomialQueue(compare_pair);
        oneRunResult_i2.impl = std::string("binomial_queue");
        oneRunResult_i2.trace_path = traceFileBaseName2;
        run_trace_ops(binomialQueue, oneRunResult_i2, operations2);
        runResults2.emplace_back(oneRunResult_i2);

        RunResult oneRunResult_i3(run_meta_data2);
        LinearBaseLine linear_base_line(compare_pair);
        oneRunResult_i3.impl = std::string("linear_base");
        oneRunResult_i3.trace_path = traceFileBaseName2;
        run_trace_ops(linear_base_line, oneRunResult_i3, operations2);
        runResults2.emplace_back(oneRunResult_i3);

    }
    if (runResults2.size() == 0) {
        std::cerr << "No trace files found.\n";
        return 1;
    }

    
    const auto csvPath = csvDir + "/" + profileName2 + ".csv";
    std::ofstream csv_out2(csvPath);
    if (!csv_out2.is_open()) {
        std::cerr << "failed to open " << csvPath << " for writing.\n";
    } else {
        std::cout << RunResult::csv_header() << std::endl;
        csv_out2 << RunResult::csv_header() << std::endl;
        
        for (const auto& run : runResults2) {
            std::cout << run.to_csv_row() << std::endl;
            csv_out2 << run.to_csv_row() << std::endl;
        }
    }
